...
//const SaveScore= JSON.parse(localStorage.getItem("SaveScore")) || [];
...
//const peplay_quiz = result_box.querySelector(".buttons .replay");
...
quit_quiz.onclick = ()=>{
    window.location.href=`/ShowResult?id=${product.id}`; //reload the current window
...
next_btn.onclick = ()=>{
    if(que_count < questions.length - 1){ //if question count is less than total question length
        ...
        skip_btn.classList.remove("hide");
        skip_btn.classList.remove("disabled");
        next_btn.classList.add("hide"); //hide the next button
...
//if Skip Que button clicked
skip_btn.onclick=()=>{
    saveData((que_count+1),4);
    next_btn.onclick();// jump next question
}
...
function optionSelected(answer){
    ...
    for(i=0;i<4;i++){
       ...
       // console.log( "Question:"+(que_count+1)+"user selected:"+userAns);
        console.log( "Question:"+(que_count+1)+"user selected:"+i);
        saveData((que_count+1),i);
...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 0){ //if timer is less than 0
            ...
            next_btn.onclick();//auto next if time over
        }
    }
}

function startTimerLine(time){
    counterLine = setInterval(timer, 86);
    function timer(){
